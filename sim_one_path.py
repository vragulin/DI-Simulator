...
def load_sim_settings_from_file(file: str) -> dict:
    ...
    # Calculate rebalance dates (arbitrarily)
    rebal_dates = sorted([config.t_last + du.relativedelta(months=-3 * x) for x in range(len(prices.T))])

    # Pack into a dictionary
    sim = {'w_idx': w_idx, 'stk_info': stk_info, 'prices': prices.T,
           'dates': rebal_dates, 'params': params}
...
def process_input_data(inputs: dict, return_override: float = None) -> dict:
    ...
    d_px = inputs['prices'].pct_change().fillna(0).replace(np.Inf, 0)
    px = (1 + d_px).cumprod()

    dt = inputs['params'].loc['dt', 'Value']
    ...
    out_dict = {'px': px, 'd_px': d_px, 'tri': tri, 'd_tri': d_tri,
...
def init_sim_hist(port: PortLots, out_dict: dict) -> list:
...
def heuristic_rebalance(port: PortLots, t: int, sim_data: dict, max_harvest: float = 0.5) -> dict:
    ...
    harvest_thresh = sim_data['params'].loc['harvest_thresh', 'Value']
...
def gen_sim_report(sim_hist: list, sim_data: dict) -> pd.DataFrame:
    ...
    rpt_cols = ['harvest', 'potl hvst', 'ratio', 'donate', 'tracking', 'port_val', 'bmk_val']
    ...
    for i in range(len(df_report)):
        ...
        df_report.loc[i, 'potl hvst'] = sim_hist[i]['opt']['potl_harvest']
    ...
    df_report['ratio'] = df_report['harvest'] / df_report['potl hvst']
...
def run_sim(input_file: str, suffix=None):
    ...
    # he index at t=0
    port = init_portfolio(inputs, sim_data)
    ...
    for t in range(1, len(sim_data['px'])):
        ...
        if params['clock_reset']:
            clock_reset_tax = port.reset_clock(reset_thresh=-params['harvest_thresh'])
        else:
            clock_reset_tax = 0
