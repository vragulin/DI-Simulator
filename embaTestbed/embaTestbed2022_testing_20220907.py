...
def load_data(data_freq: int, data_dict={}, replace=False, randomize=False, return_override=0,
    ...
    if randomize:
        ...
        d_px = out_dict['d_px'].iloc[:, full_indic].sample(frac=1, replace=replace,
        ...
        print(f"Index vol: {idx_vol*100:.2f}, tgt: {vol_override*100:.2f}")
        # **************************************************
        # End of patch
        # **************************************************
...
def run_sim(inputs: dict) -> float:
    ...
    for n in range(0, N):
        ...
        data_dict = load_data(inputs['dt'], replace=inputs['replace'], randomize=inputs['randomize'],
                              ...
                              vol_override=inputs['vol_override'], rand_state=n)
    mean_irr = float(np.mean(irr))

    return mean_irr


def run_scenario(inputs: dict, only_base: bool = False):
...
def run_scenario_hypercube(inputs: dict):
    ...
    f = open(os.path.join(dir_path, 'emba_irr_hypercube_20220910.csv'), 'w')
    header = ''
    for k in inputs:
        header += k + ','
    header += 'irr\n'
        ...
        for r in returns:
            for v in vols:
                for harvest in harvests:
                    ...
                    print('Harvest: {}, Donation: {}, Return: {}, Vol: {}, '
                          'IRR: {}'.format(harvest, donation, r, v, irr))
    f.close()
    print("Done")
